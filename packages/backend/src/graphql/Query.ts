...
const Query = {
  async product(args: { handle: string }): Promise<API.Product> {
    const data = (
      await Shopify.query<{ productByHandle: Shopify.Product }>(
        gql`
  ...
  async products(args: {
    ...
    const data = (
      await Shopify.query<{ products: Shopify.Connection<Shopify.Product> }>(
        gql`
          query products(
            $first: Int
            $after: String
