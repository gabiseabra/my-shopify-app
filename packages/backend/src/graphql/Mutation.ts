...
const Mutation = {
  async productCreate(input: {
    product: API.ProductInput
  }): Promise<API.Product> {
    // Create the product.
    // @note The product is created with a single variant.
    const product = (
      await Shopify.query<{ productCreate: { product: Shopify.Product } }>(
        gql`
  ...
  async productUpdate(input: {
    ...
    const product = (
      await Shopify.query<{ product: Shopify.Product }>(
        gql`
          query product($id: ID!) {
            product(id: $id) {
              ...ProductFragment
    ...
    // Update the product
    const result = (
      await Shopify.query<{ productUpdate: { product: Shopify.Product } }>(
        gql`
